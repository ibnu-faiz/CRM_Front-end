...
import { ArrowRight, Loader2, PackageOpen } from 'lucide-react'; // Tambah PackageOpen
...
export default function RecentDeals() {
  ...
  useEffect(() => {
    const fetchDeals = async () => {
      try {
        ...
        const res = await fetch(`${API_URL}/dashboard/recent-deals`, {
  ...
  return (
    <Card className="h-full bg-white hover:shadow-md transition-shadow flex flex-col">
      ...
      {/* CardContent Flex Column agar tombol selalu di bawah */}
      <CardContent className="px-3 pb-3 pt-0 flex-1 flex flex-col space-y-4"> 
        ...
        {loading ? (
           ...
           <div className="flex-1 flex flex-col items-center justify-center text-gray-400 border-2 border-dashed border-gray-100 rounded-lg bg-gray-50/50 py-6">
             ...
             <p className="font-medium text-gray-500">No recent deals</p>
             <p className="text-xs text-gray-400 mt-1">Start adding leads to see activity</p>
           </div>
        ) : (
          ...
          <div className="space-y-3 flex-1">
            {deals.map((deal) => (
              <div 
                key={deal.id} 
                className="p-4 border border-gray-200 rounded-xl flex items-center justify-between group hover:border-gray-300 transition-colors"
              >
                {/* KIRI: Informasi Utama */}
                <div className="space-y-1 min-w-0 pr-4 flex flex-col items-start">
                  <p className="font-semibold text-gray-900 truncate w-full" title={deal.title}>
                    {deal.title}
                  </p>
                ...
                <div className="flex flex-col items-end gap-2 shrink-0">
                  ...
                  <Link 
                    href={`/leads/${deal.id}`} 
                    className="text-xs text-gray-400 group-hover:text-blue-600 flex items-center justify-end gap-1 transition-colors font-medium"
                  >
